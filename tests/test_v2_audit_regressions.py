...
# ── Headless approval must honour the staged batch's real risk ────────────────


class TestHeadlessRiskLevel:
    """`--yes` must not auto-approve a batch containing a dangerous op."""

    # Imported inside each test so this module still collects against a
    # tree that predates staged_batch_risk_level, which is what makes
    # "does this test actually catch the bug?" checkable.

    def test_batch_with_a_deletion_reports_dangerous(self):
        pm = ChangeManager()
        pm.propose("/ws/a.py", "old", "new")
        pm.propose_deletion("/ws/b.py", "gone")

        from src.agent.headless import staged_batch_risk_level

        assert staged_batch_risk_level(pm) == "dangerous"

    def test_batch_of_content_edits_reports_staged(self):
        pm = ChangeManager()
        pm.propose("/ws/a.py", "old", "new")

        from src.agent.headless import staged_batch_risk_level

        assert staged_batch_risk_level(pm) == "staged"

    def test_empty_batch_reports_safe(self):
        from src.agent.headless import staged_batch_risk_level

        assert staged_batch_risk_level(ChangeManager()) == "safe"

    def test_headless_approve_policy_still_refuses_a_deletion_batch(self):
        pm = ChangeManager()
        pm.propose_deletion("/ws/precious.txt", "IMPORTANT")

        from src.agent.headless import ExecutionPolicy, staged_batch_risk_level

        policy = ExecutionPolicy(mode="headless", staged_policy="approve")

        assert policy.can_auto_approve(staged_batch_risk_level(pm)) is False

    def test_headless_approve_policy_still_allows_a_plain_write_batch(self):
        pm = ChangeManager()
        pm.propose("/ws/a.py", "old", "new")
        ...
        assert policy.can_auto_approve(staged_batch_risk_level(pm)) is True
...
class TestApprovedDiffMatchesDisk:
    ...
    "\\n" text and is edited as "\\n" text.  Writing it back in text mode
    rewrote the whole file's line endings while the approved diff showed
    only the line that changed — and on Windows the mirror case reflowed
    every LF file, because text-mode writes translate "\\n" to os.linesep.
    """
    ...
    def test_only_the_changed_line_differs_on_disk(self, tmp_path):
        ...
        manager = self._staged_edit(target, "a\nb\nc\nd\n", "a\nB\nc\nd\n")
        manager.apply_all()
